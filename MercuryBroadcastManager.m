...
-(void)start:(void (^)())onCompletion
{
   ...
   //NSData *data = [[NSString stringWithFormat:@"TARGET All"] dataUsingEncoding:NSUTF8StringEncoding];
   ...
   //if(![_listenSocket bindToPort:50501 error:&error])
   //   NSLog(@"Error binding: %@", error);
   //
   //if(![_listenSocket beginReceiving:&error])
   //   NSLog(@"Error receiving: %@", error);
   
   [_udpSocket sendData:data toHost:@"255.255.255.255" port:50500 withTimeout:-1 tag:777];
}
